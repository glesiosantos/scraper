...
function lerJobs(valor: string): Job[] {
  const jobs = valor
    .split(/[\n,;]/)
    .map(item => item.trim())
    .filter(Boolean)
  ...
  if (!jobs.length) throw new Error("SCRAPER_JOBS nao possui nenhum job.")
  return jobs
}

function lerHorario(valor: string) {
  const match = /^(\d{2}):(\d{2})$/.exec(valor)
  if (!match) throw new Error(`DAILY_AT invalido: "${valor}". Use HH:MM.`)
...
function executar(job: Job) {
  return new Promise<number>((resolve, reject) => {
    console.log(`\n[worker] Iniciando ${job.tipo}:${job.marca}`)

    const processo = spawn(
      process.execPath,
      [path.join(__dirname, "scraper.js"), job.tipo, job.marca],
      { stdio: "inherit" }
    )

    processo.once("error", reject)
    processo.once("exit", (codigo, sinal) => {
      if (sinal) console.error(`[worker] Processo encerrado pelo sinal ${sinal}`)
      resolve(codigo ?? 1)
    })
  })
}

async function executarTodos(jobs: Job[]) {
...
function milissegundosAteProximaExecucao(hora: number, minuto: number) {
